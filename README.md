...
[image7]: ./images/results_color_thresholds.jpg "Color binary Performance"
[image8]: ./images/signs_vehicles_xygrad_undistored.jpg "Undistored pipeline image"

## Overview
---
The main objective of this project is to classify traffic signs using a Convolutional Neural Network (CNN). The project aims to train a model based on data from [German Traffic Sign Dataset](http://benchmark.ini.rub.de/?section=gtsrb&subsection=dataset). The project reads in an image of size 32x32 and classifies them to one of the 43 possible classes. The solution is implemented using the Tensorflow deep learning library framework.
...
In addition, Udacity has provided a [CSV file](./data/signnames.csv) which has a mapping of class identifiers to their respective readable texts.
...
### Image augmentation
Image Augmentation is the process of taking images that are alreasy in a training dataset and adjusting them to create many adapted versions of the same image. This project uses `keras` deep learning library for augmenting images:
...
As seen above, the training set has an average of 2500 images per class, the amount to 107500. But the proportion between Training samples and validation sample is heavily skewed. To balance the proportion of training and validation data, i used the `split_train_test()` function from `sklearn.model_selection`
```python
